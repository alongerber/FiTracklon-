...
2. **קרא את `index.html`** — תראה מה כבר קיים
3. **הצג Audit:** מה אתה רואה? מה הקבצים העיקריים? מה הגרסה הנוכחית?
4. **אשר שאתה מבין:**
   - את ה-TODOLIST (15 שלבים: A-M)
   - את דרישות הביצועים (M)
   - את כללי העבודה
5. **תתחיל ב-שלב A.** אל תקפוץ קדימה.

ב-prompts הבאים — Alon יגיד "המשך" או "תעשה שלב X". פשוט תעשה. דווח בקצרה.
